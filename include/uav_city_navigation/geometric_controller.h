...
#include <uav_city_navigation/GeometricControllerConfig.h>
#include <std_srvs/SetBool.h>
#include <trajectory_msgs/MultiDOFJointTrajectory.h>
#include <trajectory_msgs/MultiDOFJointTrajectoryPoint.h>

#include "uav_city_navigation/common.h"
#include "uav_city_navigation/control.h"

#define ERROR_QUATERNION 1
#define ERROR_GEOMETRIC 2

// Custom Controller Modes
#define ORIGINAL_GEO_CONTROLLER 0
#define GEO_CONTROLLER_WITHOUT_POSITION_FB 1
#define GEO_CONTROLLER_WITH_POSITION_FB 2
#define MULTI_AGENT_GEO_CONTROLLER 3
...
class geometricCtrl {
 private:
  ...
  Eigen::Vector3d Omega_d_;
  double Omega_d_x_, Omega_d_y_, Omega_d_z_;
  double K_perp_, K_papral_, radius_;
  Eigen::Vector3d takeoff_pos_;

  // Dynamic consensus gains
  double k2_, k3_, k4_;
  // Map to store the global spawn offsets of all drones
  std::map<int, Eigen::Vector3d> spawn_offsets_;
  
  // RViz Marker Publisher
  ros::Publisher spin_axis_pub_;
  ...
  enum FlightState { WAITING_FOR_HOME_POSE, TAKEOFF, HOVERING, MISSION_EXECUTION, LANDING, LANDED } node_state;

  bool trajectory_received_{false};
  ros::Publisher global_odom_pub_; // To send global position to the trajectory generator
 ...
 public:
  void dynamicReconfigureCallback(uav_city_navigation::GeometricControllerConfig &config, uint32_t level);
  ...
  std::map<int, ros::Subscriber> neighbor_pose_subs_;
  std::map<int, ros::Subscriber> neighbor_twist_subs_;

  // Storage for neighbor real-time states
  std::map<int, Eigen::Vector3d> neighbor_pos_;
  std::map<int, Eigen::Vector3d> neighbor_vel_;

  // Callback functions with injected neighbor IDs
  void neighborPoseCallback(const geometry_msgs::PoseStamped::ConstPtr &msg, int neighbor_id);
  void neighborTwistCallback(const geometry_msgs::TwistStamped::ConstPtr &msg, int neighbor_id);
};
